...
import { Phone, MapPin, ShoppingBag, Component } from 'lucide-react';
...
import SpikoeImage from '@/public/products/Spikoe.png';
import RotiSosisImage from '@/public/products/Roti Sosis.png';
  ...
  const [riwayatPesanan] = useState<IRiwayatPesanan[]>([
    {
      ...
      status: 'Selesai',
      produk: [
        { id: 1, nama: 'Lapis Legit', harga: 10000 },
        { id: 2, nama: 'Lapis Surabaya', harga: 12000 },
      ],
    },
    {
      ...
      status: 'Diproses',
      produk: [
        { id: 3, nama: 'Brownies', harga: 15000 },
        { id: 4, nama: 'Mandarin', harga: 8000 },
      ],
    },
  ]);

  const [selectedPesanan, setSelectedPesanan] = useState<IRiwayatPesanan | null>(null);
          ...
          {riwayatPesanan.map((pesanan) => (
            <div key={pesanan.id_pesanan} className="relative">
              ...
              <div className="flex items-center">
                ...
                <p className="text-sm">Belanja {toIndonesiaDate(pesanan.tgl_order)}</p>
              </div>
              {/* Detail Produk */}
              <div className="p-4 bg-white shadow rounded-lg">
                    ...
                    <div key={item.id} className="flex items-center justify-between">
                      <div className="flex items-center space-x-2">
                        <div className="w-8 h-8">
                          {item.nama === 'Lapis Legit' && <Image src={LapisLegitImage} alt="Lapis Legit" />}
                          {item.nama === 'Lapis Surabaya' && <Image src={LapisSurabayaImage} alt="Lapis Surabaya" />}
                          {item.nama === 'Brownies' && <Image src={BrowniesImage} alt="Brownies" />}
                          {item.nama === 'Mandarin' && <Image src={MandarinImage} alt="Mandarin" />}
                          {item.nama === 'Spikoe' && <Image src={SpikoeImage} alt="Spikoe" />}
                          {item.nama === 'Roti Sosis' && <Image src={RotiSosisImage} alt="Roti Sosis" />}
                        </div>
                        <div>
                          <p>{item.nama}</p>
                          <p>{toRupiah(item.harga)}</p>
                        </div>
                      </div>
                    </div>
                  ))}
                ...
                <div className="mt-4 flex justify-between items-center">
                  <p className="text-sm font-semibold">Total Pesanan: {toRupiah(pesanan.total_pesanan)}</p>
