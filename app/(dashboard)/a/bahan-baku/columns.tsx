...
import { deleteBahanBakuById } from "@/lib/api/bahan-baku";

export const columns = (onRefresh?: () => void): ColumnDef<IBahanBaku>[] => [
  {
    ...
    cell: ({ row }) => (
      <div className="px-4">{row.getValue("id_bahan_baku")}</div>
    ),
  },
  {
    accessorKey: "nama",
  ...
  //   },
  {
    ...
    cell: ({ row }) => {
      ...
      const onDeleteHandler = async () => {
        try {
          setIsLoading(true);
          await deleteBahanBakuById(row.getValue("id_bahan_baku"));
          onRefresh!();
        } catch (error: any) {
