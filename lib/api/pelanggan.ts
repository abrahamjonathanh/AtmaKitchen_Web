...
import { IPelanggan } from "../interfaces";
...
export const getRiwayatPesananPelangganById = async (id: number) => {
  try {
    const response = await axios.get(`${process.env.BASE_API}/pelanggan/${id}/riwayat-pesanan`);
    toast.success("Berhasil mendapatkan riwayat pesanan pelanggan...");
    return response;
  } catch (error) {
    toast.error("Oh no! Terjadi kesalahan saat mengambil riwayat pesanan pelanggan...");
  }
};
